...
//
// **It is also unreachable.** The bound payment gateway is a fake that always succeeds, so no
// code path emits this today; it is modelled so that a real processor's decline has somewhere to
// land. Do not write a test that expects to observe it, and do not conclude from its silence that
// refunds cannot fail.
//
