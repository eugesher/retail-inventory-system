...
Status: **Superseded by ADR-027** (the `StockItem` / `product_stock` ledger is replaced by `StockLevel` running totals + a first-class `StockLocation`). Original: Accepted 2026-05-13; cache port + adapter generalized by ADR-016 → ADR-021 → ADR-022 → ADR-023; fourth `ITransactionPort` added to ring-fence `ARCH-LINT-EX-01` — see References. [ADR-012](../docs/adr/012-stock-aggregate-and-port-adapter.md)

> **Superseded — read ADR-027 for the live inventory model.** The decision and rules below describe the retired `product_stock` ledger; `IStockEventsPublisherPort` and `ITransactionPort` are the only pieces carried forward.
...
Status: **Superseded by ADR-028** (the single mutable `Order`, the one-row-per-unit `order_product` model, the two-value status, and the `INVENTORY_CONFIRM_GATEWAY` cross-service confirm flow are replaced by a mutable `Cart` + immutable `Order` chain with three orthogonal statuses, money-minor line snapshots, `Payment`, and snapshotted `Address`). Original: Accepted 2026-05-14; pipe-loader methods added to `IOrderRepositoryPort` post-ADR — see References. [ADR-013](../docs/adr/013-order-aggregate-and-cross-service-confirm.md)

> **Superseded — read ADR-028 for the live checkout model.** The decision and rules below describe the retired `order_product` order model and its `inventory.order.confirm` reserve call; the `order` / `order_product` / `order_status` / `order_product_status` tables were dropped in the ADR-028 rebuild. Nothing here is carried forward.
...
Status: Accepted (2026-06-02). [ADR-025](../docs/adr/025-catalog-product-and-variant-aggregate.md)

- **Decision:** The new `catalog` bounded context (`apps/catalog-microservice/src/modules/catalog/`) owns the merchandisable graph. `Product extends AggregateRoot<number | null>` is the write-side root and owns its `ProductVariant[]` children; `ProductVariant extends Entity<number | null>` is a child entity (sellable/stocked/priced unit). Product lifecycle is `draft → active → archived` (soft-delete via `status`, archival terminal); variant lifecycle is `active / archived`. `variantId` is the downstream backbone key (inventory stock, pricing, order lines key on the variant, not the product). State transitions record three in-process `DomainEvent`s mapped to versioned (`v1`) wire events by the use case. No optimistic-lock `version` column (catalog is last-writer-wins).
- **Binding rules for implementers:**
  - **`Product` is the only write aggregate; `ProductVariant` is a child** added/validated through the root (`Product.addVariant(...)`) — never persisted or mutated standalone on the write path. Top-level variant addressing is a **read model** (query path), not a second write aggregate.
  - **Key downstream work on `variantId`, not `productId`.** The variant is the unit with a price/stock/order-line; later inventory/retail work reshapes the plain `product_id` columns onto a catalog `variantId`.
  - **Soft-delete is via `status` only** (`draft/active/archived`); never write a `deletedAt` timestamp. The inherited `BaseEntity.deletedAt` column is left **inert** on catalog tables — `status` is the lifecycle source of truth. Archival is terminal (no `archived → draft`/`active`); `publish()` on a non-draft and `archive()` on a non-active raise `CatalogDomainException`.
  - **Domain-enforced invariants:** `Product.name`/`slug` non-empty; `ProductVariant.sku` non-empty; `optionValues` non-empty map of non-empty strings (`OptionValues` VO); `weightG` non-negative integer when present; `dimensionsMm` non-negative integer mm when present (`Dimensions` VO); `publish()` requires ≥1 variant. **`slug`/`sku` global uniqueness is repository-level** — the domain cannot see other aggregates and trusts the repository (UNIQUE constraint + use-case-spec assertion via a repo double), so don't try to enforce it in the model.
  - **Three in-process `DomainEvent<number>` subclasses** (`aggregateId` = productId): `VariantCreatedEvent` (carries `variantId: number | null`, `sku`), `ProductPublishedEvent` (`slug`, `variantIds: number[]`), `ProductArchivedEvent`. Drain via `pullDomainEvents()` and map to versioned `v1` wire events in the use case — **never serialize a `DomainEvent` subclass across services** (ADR-011/ADR-013). `VariantCreatedEvent.variantId` is `null` until persistence assigns it; the use case re-reads the concrete id from the saved aggregate. There is **no `ProductCreated` event** — `Product.create(...)` records nothing.
  - **The publish "≥1 active Price" precondition is a documented seam, not domain code.** `publish()` enforces only the variant count; the publish *use case* warns (not blocks) on a price-less product until a pricing capability lands. Don't add a price check to the domain.
  - **Lifecycle enums live in catalog `domain/`** (`ProductStatusEnum`, `ProductVariantStatusEnum`), not `libs/contracts` — they are internal domain concepts, not wire contracts (divergence from `OrderStatusEnum`). A versioned wire DTO carries its own status representation.
  - **`CatalogDomainException` (+ `CatalogErrorCodeEnum`) is the first concrete `DomainException` subclass** in the repo (older aggregates throw plain `Error`). Throw it with a typed code; the presentation layer maps the code → HTTP status — don't string-match exception messages.

## ADR-026 — `Price` append-only ledger and the `TaxCategory` label

Status: Accepted (2026-06-04). [ADR-026](../docs/adr/026-price-append-only-ledger-and-tax-category.md)

- **Decision:** The colocated `pricing` module (`apps/catalog-microservice/src/modules/pricing/`) owns price + tax-category state. `Price extends Entity<number | null>` is one row of an **append-only-for-history** ledger: a currency-scoped, time-bounded `[validFrom, validTo)` amount keyed on the opaque `variantId`. A price *change* is a **new row + a close of the predecessor's `validTo`**, never an in-place value edit — this is what satisfies the always-audit "every price change" rule. The only scope axis is `(variantId, currency)`; at most one **open** (`validTo IS NULL`) row per scope. `TaxCategory extends Entity<number | null>` is a **classification label only** (code + name; no rate/jurisdiction/conversion). Schema: `price`, `tax_category`, and a nullable `product_variant.tax_category_id` FK (`ON DELETE SET NULL`). `PricingDomainException` (+ `PricingErrorCodeEnum`) is the second concrete `DomainException` subclass (mirrors ADR-025's pattern).
- **Binding rules for implementers:**
  - **`Price` is never value-edited in place.** Two construction paths: `Price.set({...}, now?)` is the write path — `validFrom` defaults to now and a `validFrom` **strictly before now** is rejected (`PRICE_VALID_FROM_IN_PAST`); `Price.reconstitute({...})` is the load path (any `validFrom`, no guard). `close(at)` is the **only** mutation — it returns a *new* `Price` with the value fields copied and `validTo = at`; `amountMinor`/`currency`/`variantId`/`priority` are immutable (no setters). A value change = append a new open row + `close` the predecessor.
  - **`variantId` is an OPAQUE link** — the pricing domain MUST NOT import the catalog `Product`/`ProductVariant` (forbidden cross-module import, ADR-004/ADR-017). The only coupling is the `FK_PRICE_VARIANT` foreign key (`price.variant_id → product_variant.id`, `ON DELETE RESTRICT`) in the migration. `PriceEntity` maps `variant_id` as a **plain BIGINT scalar with no `@ManyToOne`**.
  - **At-most-one-open is enforced in two layers.** Primary: the repository's `appendPrice(newPrice, predecessorToClose)` runs the close-UPDATE + the insert in **one** `manager.transaction(...)`, then re-reads by id for the concrete generated id (the "re-read the saved graph" idiom). Backstop: a `STORED` generated column `open_scope_key` (non-NULL only while `valid_to IS NULL`) under a UNIQUE index emulates a partial unique index MySQL lacks — closed rows (NULL key) never collide, two open rows for one scope do. **Don't map `open_scope_key` on the entity** — it is a DB-internal backstop; `synchronize` is off and an insert that omits it lets MySQL compute it.
  - **Select Applicable resolution lives in the use case, not the repository/SQL.** `findInEffect(variantId, currency, asOf)` returns the **coarse** candidate set (rows whose `[validFrom, validTo)` contains `asOf`); the **pick policy = highest `priority`, then latest `validFrom`** is realized in the price use case (so it stays unit-testable and schema-free).
  - **`IPricingRepositoryPort` (`PRICING_REPOSITORY`) returns domain types only** — no `typeorm` leak (ADR-017). `PricingTypeormRepository` is the single `@InjectRepository` site for the context. `PriceMapper.toDomain` coerces the BIGINT `variant_id`/`amount_minor` strings back to numbers with `Number(...)` (mysql2 returns non-PK BIGINTs as strings); `amountMinor` is **minor units (integer cents)**, never a float.
  - **`TaxCategory` carries no rate/jurisdiction** — classification only; tax computation is a deferred future capability. Global `code` uniqueness is **repository-level** (`UC_TAX_CATEGORY_CODE` UNIQUE + a use-case `findTaxCategoryByCode` pre-check raising `TAX_CATEGORY_CODE_TAKEN`), not model-enforced — the catalog `slug`/`sku` convention.
  - **Throw `PricingDomainException` with a typed `PricingErrorCodeEnum` code** (the code is a *property*, not in the message — assert `err.code`, don't string-match). `deletedAt` stays **inert** on `price`/`tax_category` — pricing never soft-deletes.

## ADR-027 — `StockLevel` running totals and location-aware `StockLocation`

Status: Accepted (2026-06-08; supersedes ADR-012). [ADR-027](../docs/adr/027-stocklevel-running-totals-and-stocklocation.md)

- **Decision:** The inventory `stock` context drops the append-only `product_stock` ledger and re-founds on two aggregates: per-location **`StockLevel` running totals** (`quantityOnHand` / `quantityAllocated` / `quantityReserved`, with `available = onHand − allocated − reserved` a pure getter) keyed on `(variantId, stockLocationId)`, and a first-class **`StockLocation`** aggregate (`stock_location` string-PK table: `name`, `code`, `type` = warehouse/store/dropship-virtual, optional JSON `address` + 13-digit `gln`, `active` flag). A read becomes a point lookup of one `stock_level` row, not a `SUM/GROUP BY` re-aggregation that grows with history. `stock_level` ships a `version` optimistic-lock column **now** (consumed by the later reservation capability). All inventory keys move from `productId` to a cross-service `variant_id` FK → `product_variant(id)`. One migration drops `product_stock` / `product_stock_action` / `storage`, creates `stock_location` + `stock_level`, and auto-provisions a `default-warehouse` location idempotently; no production data exists, so it is a clean cut, not an in-place migration.
- **Binding rules for implementers:**
  - **Running totals are the source of truth, not a ledger.** `StockLevel` persists maintained counters; `available` is the pure getter `onHand − allocated − reserved` — never re-derive a balance by summing movement rows. An audit trail (a `StockMovement`-style log) belongs to the later inventory-reservation capability and is **not** the balance authority.
  - **The aggregate exposes only `changeOnHand(delta)`** (rejects a negative result, bumps `version`) plus a `StockLevel.initialAt(variantId, stockLocationId)` zeroed factory. `allocate` / `reserve` / `release` are intentionally **absent** — don't add them in this foundation; they'd be dead, untested code until reservation lands.
  - **`StockLocation` has a caller-assigned string PK** (e.g. `default-warehouse`) — it diverges from `BaseEntity`'s auto-increment integer id (ADR-019). Soft-delete is via the `active` flag, **never** a `deletedAt` timestamp (which stays inert, as in catalog/pricing). `code` global uniqueness is **repository-level** (UNIQUE constraint), not model-enforced — the catalog `slug`/`sku` convention (ADR-025). Exactly one default location is auto-provisioned by the migration (`INSERT ... ON DUPLICATE KEY UPDATE`, the Vendure "always a default location" stance); don't special-case a "no location" path.
  - **The `version` column ships now** even though no concurrency guard consumes it yet — retrofitting an optimistic-lock column onto a populated table later is a destructive `ALTER TABLE`. The aggregate bumps its in-memory `version` on every mutation; persistence uses TypeORM `@VersionColumn()`.
  - **`variant_id` is an OPAQUE cross-service FK** (`→ product_variant(id)`, `ON DELETE RESTRICT`) — the inventory domain MUST NOT import the catalog `ProductVariant` (forbidden cross-module import, ADR-004/ADR-017; same precedent as pricing ADR-026). `StockLevelEntity` maps `variant_id` as a **plain BIGINT scalar with no `@ManyToOne`**. Non-negativity is also backed by three MySQL 8.4 `CHECK` constraints.
  - **The cache *mechanism* is preserved; only the cached *value shape* changes.** Cache-aside (ADR-002 → ADR-006 → ADR-016 → ADR-021 → ADR-022 → ADR-023) stays; the cached value becomes a per-variant `StockLevel` projection, not a per-product `SUM`-aggregate. This foundation **deletes the old `StockCache`** and leaves `cache-keys.ts` at `v1`; the `StockCache` rebuild + the key-version bump that records the new shape land with the later availability read path.
  - **`IStockRepositoryPort` is rewritten** to `findLocation` / `listLocations` / `findStockLevel` / `findStockLevelsByVariant` / `saveStockLevel` (domain types only, no TypeORM leak — ADR-017). `IStockEventsPublisherPort` and `ITransactionPort` from ADR-012 are **retained**; the repository port is the only ADR-012 port rewritten.
  - **The microservice boots with only the `inventory.order.confirm` deprecation stub** (a typed `RpcException`); the read/availability RPC, gateway endpoints, the auto-init consumer, and Receive/Adjust land in later capabilities. The `inventory.product-stock.get` routing key is **removed**; the `IProductStockOrderConfirmPayload` confirm contract is **kept** so the retail confirm flow still type-checks (the whole confirm seam is removed when the reservation capability lands).

## ADR-028 — Cart, Order, Payment, and Address: the rebuilt checkout chain

Status: Accepted (2026-06-10; supersedes ADR-013). [ADR-028](../docs/adr/028-cart-order-payment-and-address-chain.md)

- **Decision:** The retail microservice's checkout context is rebuilt as **two aggregates in one bounded context** — a **mutable `Cart`** (root + `CartLine` children, the shopper's editable working set) and an **immutable `Order`** (root + `OrderLine` children, the placed record). Placing is a **one-shot conversion**: the cart's lines are snapshotted into order lines (variant id, quantity, unit/line money amounts as they stood at that instant) and the cart is marked `converted`, so **no later cart edit can corrupt a placed order**. `Order` carries **three orthogonal status axes** (`status` = pending/confirmed/cancelled/shipped/delivered; `paymentStatus` = none/authorized/captured/refunded/failed; `fulfillmentStatus` = unfulfilled/partially-shipped/shipped/delivered) that evolve independently. Payment **authorizes on place** (inline, through a `PAYMENT_GATEWAY` port) and **captures explicitly** (a separate operation). The **`Payment` aggregate + the `PAYMENT_GATEWAY` port + its default `FakePaymentGatewayAdapter` live inside the `orders/` module** (Payment's operations act on `Order`; a standalone module would only buy cross-module coupling). A polymorphic **`Address`** (`ownerType ∈ {customer, order}`) is **snapshotted** as immutable `ownerType=order` copies onto each placed order. Supersedes ADR-013's single mutable `Order` / one-row-per-unit `order_product` / two-value status / `inventory.order.confirm` reserve model; because no production data exists, the legacy model is a **clean cut** (the four `order*` tables dropped), not an in-place refactor.
- **Binding rules for implementers:**
  - **Cart is mutable, Order is immutable — never edit a placed order through the cart.** Placement is the one-shot conversion at place-time; an `OrderLine` is a frozen place-time snapshot (`Object.freeze`-d), and both `CartLine` and `OrderLine` key on the **opaque `variantId`** (never import the catalog `ProductVariant`). A single mutable `Order` edited in place was rejected — it would let post-purchase churn rewrite the historical record.
  - **Three orthogonal status fields on `Order`, never one combined enum.** Payment progress and fulfillment progress evolve independently of the order lifecycle (a `confirmed` order can be `authorized` + `unfulfilled`); collapsing them forces illegal intermediate states. Only the payment axis has mutators today (`markPaymentAuthorized` / `markPaymentCaptured`) — no ship/deliver/cancel mutators yet.
  - **Authorize-on-place, capture-explicit.** Place auto-authorizes inline; Capture is a separate operator/fulfillment action. Ship-triggered auto-capture, partial capture, and void/refund/fail are later capabilities — don't add them to this chain.
  - **`PAYMENT_GATEWAY` is a port with a fake default, colocated in `orders/`.** The default `FakePaymentGatewayAdapter` always approves with deterministic `fake_<uuid>` references; a real gateway swaps in by **rebinding the port** (no use-case change), with the `ClientProxy`/HTTP client confined to the new adapter (ADR-004/017). Keep `Payment` + the port + the adapter in the orders module, not a standalone `payment/` module.
  - **An order's `Address` rows are copies, not references** into a customer address book. `Address.forOrder(...)` writes immutable `ownerType=order` snapshots from whatever the buyer supplied (`country` upper-cased 2-char ISO). A reusable `ownerType=customer` address book is a later capability the polymorphic column already accepts — no schema change needed.
  - **`version` (OCC) ships now but is unenforced; `Idempotency-Key` is accepted but not deduped.** `cart` and `order` carry a `version` column up front (retrofitting OCC onto a populated table is a destructive `ALTER`, the ADR-027 `stock_level.version` reasoning), with no guard consuming it yet. **Repeat-place idempotency is cart-state-driven** — a `converted` cart re-place returns the order it already converted into (via `order.source_cart_id`), not a second order; re-capture idempotency is payment-state-driven. A persisted idempotency-key store is deferred.
  - **Customer cart/order routes are authorized by authentication + an ownership check in the use case, NOT a customer permission code.** Customer tokens carry no `permissions` claim (ADR-024), so a `@RequiresPermission('customer:…')` gate would reject the customers it targets. The use case compares the authenticated principal's id to the resource's `customerId`. Staff overrides ride the existing model: `order:read` (existing) reads any order, `order:capture` (new) gates the staff capture — a permission code is a **staff override** over the owner-check, never a customer gate.
  - **The retail-side cross-service confirm flow is retired.** `INVENTORY_CONFIRM_GATEWAY` + `ConfirmOrderUseCase` are deleted; retail no longer calls `inventory.order.confirm`. The inventory `inventory.order.confirm` deprecation stub + `IProductStockOrderConfirmPayload` stay a **reserved surface owned by ADR-027** (removed when the inventory-reservation capability lands, not here).
  - **The six legacy `retail.order.*` keys** (`.create` / `.confirm` / `.get` / `.created` / `.confirmed` / `.cancelled`) are **retired** from `ROUTING_KEYS` + the mirrored `MicroserviceMessagePatternEnum`, replaced by `retail.cart.*` / `retail.order.placed` / `retail.payment.*` keys that arrive with their producers (the ADR-008 value-for-value agreement is upheld). `retail.order.placed` is the only retail event with a live consumer (notification, re-pointed off the retired `retail.order.created`); the `retail.cart.*` + `retail.payment.*` events are reserved surfaces.
