...
split. The Customer half is covered in §6 below; both halves share this
file because the two aggregates are defined by what they are *not* to
each other.
...
The pre-existing `user` table doubled as **both** a workforce identity (the
...
2. **Audit-log discipline is impossible.** Every action attributed to a
   ...
   distinct from any buyer ID — this is the audit-discipline requirement
   (Q7). With a single `user.id` namespace, deleting a buyer
...
- **`last_login_at TIMESTAMP NULL` joins the table.** A "last login"
  ...
  `last_login_at` is the cheap freshness signal, not the audit source
  of truth.
...
- **`RoleEnum` is now just the typed registry of *seeded* names.**
  ...
  source of truth for *which roles exist* is the `role` table — the IAM
  admin tooling can insert a new row at runtime without a
  redeploy. `RoleEnum` is a developer ergonomic, not the schema.

The `RoleVO` value object that used to wrap a `RoleEnum` is **deleted**
as part of this work. `StaffUser.roles: RoleAggregate[]` references
...
a second lookup, which is what makes the login-time JWT permission
inflation cheap.
...
Dropping the table is explicitly permitted:

> *"destructive changes to the old `user` table are permitted (no data
> to preserve)"*

— the migration strategy for the identity baseline. There is no
production deploy yet; the only inhabitants of `user` were two seeded
test fixtures, and one of them (`customer@example.com`) is moving to the
new `customer` table anyway. A schema-rewrite migration that drops and re-creates is
...
restores the previous schema cleanly.

## 5. Forward references

- **JWT inflation.** The access JWT payload will gain a
  ...
  can read `request.user.permissions` without a per-request
  DB hit. The latency-vs-freshness trade-off is recorded in ADR-024
  §Decision-3.
- **Customer half.** This document's second half (§6 below) covers the
  Customer aggregate: it lands in its own `customer` table, registration
  becomes public for buyers (`POST /api/auth/customer/register`), and the
  deferred `customer@example.com` seed re-appears against that table.
- **Future ConsentRecord + tombstone work.** GDPR-style
  erasure of a `staff_user` row is *not* implemented here — the
  policy decision (full delete vs. tombstone vs. consent-scoped
  retention) is deferred to future consent-and-erasure work. For now,
  `deleted_at` is a soft tombstone that keeps audit-log FKs resolvable;
  that future work will decide what follows.

## 6. The customer half

The buyer-side identity baseline adds a new `customer`
...
customer side. The pre-existing `user.roles: simple-array` column made one
...
one accepts a future flow that this baseline does not implement:

- **`password_hash VARCHAR(255) NULL`** — the column is nullable so a
  future guest-checkout row can exist without a password, and so
...
- **`status ENUM('active','suspended','guest','deleted')`** — a
  ...
  on day one: `'guest'` for future guest checkout (which produces a
...
Future checkout work extends the order flow so a checkout produces a
`customer` row even when the buyer is anonymous (guest checkout). That
row lands with `status='guest'`, `password_hash=NULL`, and only the PII
