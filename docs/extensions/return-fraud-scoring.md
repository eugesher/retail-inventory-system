...
[fraud-and-risk-scoring.md](fraud-and-risk-scoring.md) — the same external `RISK_SCORING_GATEWAY` port and
...
- The threshold: the first time return losses are large enough to justify an external scoring provider —
  typically once order-side [fraud scoring](fraud-and-risk-scoring.md) already exists and the same seam can
  be reused.
...
- **The `RISK_SCORING_GATEWAY` seam** ([fraud-and-risk-scoring.md](fraud-and-risk-scoring.md)) — the
...
[fraud-and-risk-scoring.md](fraud-and-risk-scoring.md) and reuses the RMA lifecycle unchanged — the new
