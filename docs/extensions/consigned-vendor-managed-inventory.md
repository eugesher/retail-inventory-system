...
[supplier and vendor](supplier-and-vendor.md) guide owns that party, and this guide resolves the one
...
- **The `Supplier` party — defined in [supplier and vendor](supplier-and-vendor.md), not here.** That
...
[supplier and vendor](supplier-and-vendor.md), which is what keeps it from being subsystem-scale on its
own.
