...
- **Pricing has exactly one scope axis, and location is the named omission.**
  ...
  [`customer-group-and-tiered-pricing.md`](../pricing-and-promotions/customer-group-and-tiered-pricing.md) describes from the
...
- **A cash tender is a `Payment` with `method: 'cash'`.** `Payment.method` is an opaque non-empty
  ...
  [`gift-cards-and-store-credit.md`](../order-management/gift-cards-and-store-credit.md) applies to store credit applies
...
- **A planogram slot is a merchandising position, not a picking position.**
  [`bin-aisle-shelf.md`](../inventory/bin-aisle-shelf.md) describes the sub-location axis for directed put-away
...
- **New permission codes**, seeded in the same change: opening a drawer, voiding a line, applying a
  ...
  [`scoped-tenant-aware-roles.md`](../staff-and-access-control/scoped-tenant-aware-roles.md)'s subject, and a manager override at
  the counter is [`approval-workflows.md`](../staff-and-access-control/approval-workflows.md)'s.
...
- **Who the authenticated principal is at a terminal.** Today there is one subject per token. A till
  ...
  ([`session-device-management.md`](../staff-and-access-control/session-device-management.md) proposes the storage a shared
  terminal would need).
- **How a cashier session relates to a rostered shift.** They are not the same thing — a session is a
  ...
  assuming; [`staff-scheduling-and-shifts.md`](../staff-and-access-control/staff-scheduling-and-shifts.md) owns the other half.
