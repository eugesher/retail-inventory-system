...
| ~~`CacheHelper`~~ | Backwards-compat shim delegating to `CACHE_KEYS`, "kept for one release". **Deleted by [ADR-046](046-libs-layout-and-dead-export-removal.md)**, three key-versions late — see the amended §Open note below. |
...
- ~~`CacheHelper`'s removal~~ — **DONE ([ADR-046](046-libs-layout-and-dead-export-removal.md), 2026-07-12).** This note had queued the deletion "behind the next cache-key version bump"; the inventory key has since gone `v1 → v2 → v3`, so the condition was met three times over and nobody acted. The shim is gone. Its only caller of `CACHE_KEYS.productStock` went with it, so that full-key builder is deleted too — the invalidate path only ever needed `productStockPrefix`, which stays.

  The lesson is the one ADR-046 draws: a removal queued behind a condition, with no owner and no check, is not queued — it is forgotten. A `CacheHelper` that no code imported still cost something, because the repo's map listed it as live API.
