...
export class PublishProductUseCase {
  ...
  public async execute(productId: number, correlationId: string): Promise<ProductView> {
    ...
    try {
      this.logger.info({ productId }, 'Publishing catalog product');

      const product = await this.catalogGateway.publishProduct(productId, correlationId);
      ...
      this.logger.error(error, 'Error publishing catalog product');
