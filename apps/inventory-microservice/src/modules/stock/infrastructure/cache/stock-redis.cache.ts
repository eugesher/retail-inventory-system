...
@Injectable()
export class StockRedisCache implements IStockCachePort {
  constructor(
    @Inject(CACHE_MANAGER)
    private readonly cache: Cache,
    private readonly configService: ConfigService,
    @InjectPinoLogger(StockRedisCache.name)
  ...
  public async get(
    ...
    const cacheKey = CACHE_KEYS.productStock(productId, storageIds);
  ...
  public async invalidate(payload: IStockCacheInvalidatePayload): Promise<void> {
    ...
    // see libs/cache/cache-module.config.ts) the prefix is empty and stored
    ...
    try {
      ...
      await Promise.all(
        productIds.map(async (productId) => {
          const pattern = `${keyPrefix}${CACHE_KEYS.productStockPrefix(productId)}*`;
  ...
  // backends. Either accept-and-document as best-effort (the call site above
  // already says "best-effort"), or enumerate combo keys via the
  // in-memory adapter's iterator interface.
  private async invalidateNamedKeys(
    items: IStockCacheInvalidatePayload['items'],
    correlationId?: string,
  ): Promise<void> {
    const cacheKeys = new Set<string>();

    for (const { productId, storageId } of items) {
      cacheKeys.add(CACHE_KEYS.productStock(productId));
      cacheKeys.add(CACHE_KEYS.productStock(productId, [storageId]));
    }
