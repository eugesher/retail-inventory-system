...
// The service's liveness probe (ADR-044), answered on its existing `catalog_queue` RPC queue —
...
export class HealthController {
  @MessagePattern(ROUTING_KEYS.CATALOG_HEALTH_PING)
  public ping(): IHealthPingResult {
    return { status: 'ok', service: AppNameEnum.CATALOG_MICROSERVICE };
  }
}
