...
// `deletedAt`. `deletedAt` stays **INERT** — a delivery row is never soft-deleted, and that is
// deliberate: the row IS the dedupe anchor (`delivery_dedupe_key`), so a hidden-but-present row the
// dedupe query no longer sees would let the same notification send twice. **The retention sweep HARD-
// deletes instead** (`PurgeAgedDeliveriesUseCase`, nightly, bounded, `RETENTION_DELIVERY_DAYS`) — the
// table was unbounded until ISSUE-08, when that config key finally got a reader.
//
