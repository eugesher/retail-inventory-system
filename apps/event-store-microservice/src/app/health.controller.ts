...
// The service's liveness probe (ADR-044), answered on its existing `event_store_query_queue` RPC queue —
...
export class HealthController {
  @MessagePattern(ROUTING_KEYS.AUDIT_HEALTH_PING)
  public ping(): IHealthPingResult {
    return { status: 'ok', service: AppNameEnum.EVENT_STORE_MICROSERVICE };
  }
}
