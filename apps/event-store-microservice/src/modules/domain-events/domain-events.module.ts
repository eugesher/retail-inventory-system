...
// ADR-035), and its read surface (ADR-039). It owns the append-only `domain_event` table
// (ADR-034 isolated `ris_eventstore` schema).
//
...
// through this export. `QueryDomainEventsUseCase` is exported for the same reason: the
// query controller is a context-level concern, not a per-module one.
@Module({
  ...
  exports: [DOMAIN_EVENT_REPOSITORY, IngestDomainEventUseCase, QueryDomainEventsUseCase],
})
