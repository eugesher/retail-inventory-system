...
class FakeTraceDomainEventReader implements ITraceDomainEventReaderPort {
  public readonly rows: ITraceDomainEventRow[] = [];
  public lastCorrelationId: string | null = null;

  public seed(...rows: ITraceDomainEventRow[]): void {
    this.rows.push(...rows);
  }

  public listByCorrelationId(correlationId: string): Promise<ITraceDomainEventRow[]> {
    this.lastCorrelationId = correlationId;
    return Promise.resolve(
      this.rows.filter((row) => row.correlationId === correlationId).sort(byOccurredAtThenId),
    );
...
const eventRow = (overrides: Partial<ITraceDomainEventRow> = {}): ITraceDomainEventRow => ({
  id: 1,
...
describe('TraceByCorrelationUseCase', () => {
  ...
  let reader: FakeTraceDomainEventReader;
  ...
  beforeEach(() => {
    ...
    reader = new FakeTraceDomainEventReader();
    logger = makePinoLoggerMock();
    useCase = new TraceByCorrelationUseCase(repository, reader, logger as unknown as PinoLogger);
  });

  it('issues both reads with the targetCorrelationId, not the request correlationId', async () => {
    await useCase.execute(payload());

    expect(reader.lastCorrelationId).toBe(TARGET);
    expect(repository.lastCorrelationId).toBe(TARGET);
    expect(reader.lastCorrelationId).not.toBe(REQUEST);
  });
  ...
  it('yields two empty arrays for an unknown correlation id, and does not throw', async () => {
    reader.seed(eventRow({ correlationId: 'some-other-request' }));
  ...
  it('projects the reader row occurredAt Date to an ISO-8601 string in the view', async () => {
    reader.seed(eventRow({ occurredAt: new Date('2026-06-27T10:00:00.123Z') }));
