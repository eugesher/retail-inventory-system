...
//
// Both logs are aggregates of THIS module (ADR-042), so the trace simply injects both
// repository ports and calls the same `listByCorrelationId` on each. The two reads touch
// different tables, so they run concurrently.
//
