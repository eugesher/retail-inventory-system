...
// rather than a lossy `''` — the row is still attributable, just not normalized. Takes the
// pre-split tokens so the caller splits the key ONCE for both this and `resolveAggregateType`.
