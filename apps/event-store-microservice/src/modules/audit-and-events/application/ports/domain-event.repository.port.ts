...
export interface IDomainEventRepositoryPort {
  ...
  // The correlation TRACE read — the exact mirror of `IAuditLogRepositoryPort`'s: every
  // firehose event belonging to one request's causal chain, ordered `occurred_at ASC, id ASC`.
  //
  // Deliberately UNPAGINATED and ASCENDING, and therefore not expressible as a call to
  // `query` above: a correlation id scopes exactly one request, so the set is bounded and
  // there is nothing to page, and a timeline reads FORWARD — the opposite of the newest-first
  // order every other read on this port wants.
  //
  // `domain_event.correlation_id` is `NOT NULL DEFAULT ''` (MySQL treats NULLs as distinct
  // inside a UNIQUE, so the ingest dedupe key needs a non-null sentinel). An event ingested
  // without a correlation id therefore stored `''` and is reachable by no trace. An unknown
  // id yields `[]`, never an error.
