...
// `publishReturnRequested` / `publishReturnAuthorized` / `publishReturnReceived` emit
// `retail.return.requested` / `.authorized` / `.received` onto `notification_events` (the
// notification service's own queue — it consumes the buyer-facing ones for its returns
// fan-out); `publishReturnRejected` / `publishReturnClosed` emit `retail.return.rejected`
// / `.closed` onto `retail_queue` (the producer's own queue — reserved surfaces today, no
// consumer). Each is a best-effort post-commit emit — the write has already committed, so
// a publish failure is warn-logged and swallowed by the caller (ADR-020).
