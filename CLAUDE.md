...
**Cache-key convention** (ADR-016). Every cache key follows `ris:<service>:<aggregate>:<id>[:<facet>]`. Apps under `apps/*/src` MUST NOT write cache-key string literals — call a builder on `CACHE_KEYS` from `@retail-inventory-system/cache`. Apps MUST NOT import `@nestjs/cache-manager`, `@keyv/redis`, or the `cacheable` package directly — depend on `ICachePort`/`CACHE_PORT` (or a domain-shaped wrapper port like `IStockCachePort`). Write paths invalidate via `CACHE_KEYS.<aggregate>Prefix` + `port.delByPrefix`, awaited post-commit so the next read sees fresh data.
