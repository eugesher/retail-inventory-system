...
The **`Fulfillment` aggregate foundation** is now in place ([ADR-031](docs/adr/031-fulfillment-aggregate-and-ship-triggered-capture.md)) — the data + domain layer for the ship flow, with its operations (Create / Ship / Deliver / Cancel) arriving in later work. A `Fulfillment` (a DB-assigned `BIGINT` id, owning its `FulfillmentLine` children) is a fourth sibling aggregate in the `orders/` module: the **per-shipment, per-location** record that drives an order toward `delivered`. It ships from an opaque inventory `stockLocationId` (retail never imports inventory), and its lines say which `OrderLine` quantities are in this box — so **partial and split shipments are simply multiple `Fulfillment`s per order**. `Fulfillment.status` (`pending → shipped → delivered`, or `cancelled`) is a **fourth status axis** beside the order's three, evolving per shipment; the order's own `fulfillment_status` is the roll-up across its fulfillments. The aggregate enforces its own shape (≥1 line, positive line quantities, legal transitions, and tracking-required-on-ship) while the cross-fulfillment sum (a line's shipped quantity across all shipments ≤ ordered) belongs to the later Create use case. It is **append-only** — cancellation is a `status` flip, never a delete (a `shipped`/`delivered` fulfillment is never cancellable, which is what will protect a future Cancel Order). The `fulfillment` / `fulfillment_line` tables, the `FulfillmentTypeormRepository` (`FULFILLMENT_REPOSITORY`), the `FulfillmentStatusEnum` + `FulfillmentView`, and the eight new `OrderErrorCodeEnum` codes (all mapped in the orders RPC exception filter) are live.
