...
| `IDEMPOTENCY_KEY_TTL_HOURS` | `24` | Idempotency-key retention (hours) — a stored idempotency record is purge-eligible once `created_at` + this many hours has passed (Joi-validated integer ≥ 1, [ADR-036](docs/adr/036-idempotency-key-store-and-enforced-occ.md)). |
