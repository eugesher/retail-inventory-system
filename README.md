...
├── modules/returns/                           # the RMA bounded context (ADR-032) — the lifecycle is live (RMQ-only, no gateway route yet)
│   ├── domain/                                # ReturnRequest + ReturnLine aggregate, ReturnDomainException + ReturnErrorCodeEnum
│   ├── application/
│   │   ├── ports/                             # RETURN_REQUEST_REPOSITORY + RETURN_ORDER_READER + RETURN_EVENTS_PUBLISHER + RETURN_WINDOW_DAYS token
│   │   └── use-cases/                         # Open/Authorize/Reject/Receive/Close + Get/List + return-view.factory + return-access helper
│   ├── infrastructure/
│   │   ├── persistence/                       # return_request/return_line entities, mappers, repo, ReturnOrderReaderTypeormAdapter (raw SQL)
│   │   ├── messaging/                         # ReturnRabbitmqPublisher (two clients: notification_events + retail_queue)
│   │   └── returns.module.ts                  # forFeature + reader/publisher/window bindings + 7 use cases + APP_FILTER (ReturnRpcExceptionFilter)
│   └── presentation/                          # returns.controller.ts (7 @MessagePatterns) + return-rpc-exception.filter.ts
└── main.ts                                    # first import: @retail-inventory-system/observability/tracer
```
